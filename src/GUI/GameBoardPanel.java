...
public class GameBoardPanel extends JPanel {
	...
	public GameBoardPanel() {
		...
		buttonPressedIcon = DropQuoteGUI.getFileInterface().loadImageIcon(Config.BUTTON_PRESSED_IMAGE);
		buttonOnIcon = DropQuoteGUI.getFileInterface().loadImageIcon(Config.BUTTON_ON_IMAGE);
		buttonOffIcon = DropQuoteGUI.getFileInterface().loadImageIcon(Config.BUTTON_OFF_IMAGE);
		buttonEmptyIcon = DropQuoteGUI.getFileInterface().loadImageIcon(Config.BUTTON_EMPTY_IMAGE);
		string = "This is a test of the emergency broadcast System";


		DropQuoteGUI.setEnglish();
		DropQuoteGUI.processor = new WordProcessor(string.toUpperCase());
		wordArray = DropQuoteGUI.processor.splitWord(NO_COLUMNS);
		sortedArray = DropQuoteGUI.processor.randomizeColumns(wordArray);
		NO_ROWS = DropQuoteGUI.processor.getRows();
		init();
	}
	/**
	...
	public void createCluePanel(){
		...
		for(int i = 0; i < NO_ROWS*NO_COLUMNS; i++){
			...
			clueButtons[i/NO_COLUMNS][i%NO_COLUMNS].setFont(DropQuoteGUI.getFileInterface().getFont());
	...
	public void createSolutionPanel(){
		...
		for(int i = 0; i < NO_ROWS*NO_COLUMNS; i++){
			...
			solutionButtons[i/NO_COLUMNS][i%NO_COLUMNS].setFont(DropQuoteGUI.getFileInterface().getFont());
			...
			if(i < DropQuoteGUI.processor.getLogicalChars().size()){
				if(DropQuoteGUI.processor.getLogicalChars().get(i).equals(" ")){
	...
	public void populateCluePanel(){
		sortedArray = DropQuoteGUI.processor.sortSpaces(sortedArray);
		for(int i = 0; i < NO_ROWS * NO_COLUMNS; i++){
			solutionButtons[i/NO_COLUMNS][i%NO_COLUMNS].setFocusable(false);
			clueButtons[i/NO_COLUMNS][i%NO_COLUMNS].setText(sortedArray[i/NO_COLUMNS][i%NO_COLUMNS]);
			clueButtons[i/NO_COLUMNS][i%NO_COLUMNS].setIcon(buttonOffIcon);
 ...
 */
	...
	public class ShuffleButtonActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e){
			sortedArray = DropQuoteGUI.processor.randomizeColumns(sortedArray);
			populateCluePanel();
		}
	}
	...
	private class ClueButtonMouseListener implements MouseListener{
		...
		public void mouseEntered(MouseEvent e) {
			...
			for (int i = 0 ; i < NO_ROWS*NO_COLUMNS ; i++){
				//check if a solution button is already clicked
			...
			if(!((MyButton)e.getSource()).isEmpty() && !foundClick){
				...
				/*
				int row = ((MyButton)e.getSource()).getRow();
				int column =((MyButton)e.getSource()).getColumn();
				System.out.println(row +" " + column);
				if(column == 0){
					
					if(clueButtons[row][column+1].isPressed()){
						((MyButton)e.getSource()).setPressed(true);
					...
					if(clueButtons[row][column-1].isPressed() || clueButtons[row][column+1].isPressed){
						((MyButton)e.getSource()).setPressed(true);
						((MyButton)e.getSource()).setIcon(buttonPressedIcon);
					}
				}
		...
		public void mouseReleased(MouseEvent e) {
			((MyButton)e.getSource()).setPressed(false);
			...
			DropQuoteGUI.getFileInterface().playSound(Config.BUTTON_CLICKED_SOUND, false);
			for (int i = 0 ; i < NO_ROWS*NO_COLUMNS ; i++){
				if(clueButtons[i/NO_COLUMNS][i%NO_COLUMNS].isClicked()){
					//check if a clue button is already clicked
					if(clueButtons[i/NO_COLUMNS][i%NO_COLUMNS].getRow() == ((MyButton)e.getSource()).getRow() &&
